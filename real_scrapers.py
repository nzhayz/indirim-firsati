...
def scrape_all_sites(categories: List[str], sites: List[str] | None = None, delay_sec: float = 1.5) -> List[Dict]:
    ...
    for category in categories:
        ...
        for site_name in sites:
            ...
            if not scraper_fn:
                continue
            try:
                results = scraper_fn(query, category)
                print(f"[real_scrapers] {site_name} / {category}: {len(results)} ürün bulundu")
