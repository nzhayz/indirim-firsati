...
def real_scrape(categories: List[str]) -> List[Product]:
    """Gerçek kazıma modu giriş noktası. Şu an şablon olarak boş çalışır;
    _scrape_site_template içindeki seçicileri doldurduktan sonra
    kategori -> arama URL'si eşlemesi ekleyerek burayı aktif hale getir."""
    all_products: List[Product] = []
    # ÖRNEK: category_url_map = {"Telefon": "https://site.com/arama?q=telefon", ...}
    # for cat in categories:
    #     url = category_url_map.get(cat)
    #     if url:
    #         all_products.extend(_scrape_site_template(url))
    return all_products
